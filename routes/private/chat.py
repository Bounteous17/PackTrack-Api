...
parser.add_argument('username', help = 'User with whom to start the chat', required = False)
...
class Chat(Resource):
    ...
    def post(self):
        try:
            ...
            newChat.save()
            uUser = _mUser.findOneAndUpdate('_id', creator, 'chats', newChat.id)
    ...
    def get(self):
        try:
            ...
            return _functions.setModuleSuccess(payload=userChats, key='list', status=200).flaskResp()
        except Exception as e:
